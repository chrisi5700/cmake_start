...
static void bm_sum_vector(benchmark::State& state) {
    ...
    std::vector<int> vec(state.range(0), 1); // Vector of 1's of size range(0)

    // Loop over each state iteration (measures performance over multiple runs)
    for (auto _ : state) {
        benchmark::DoNotOptimize(sum_vector(vec)); // Perform the sum and prevent optimization
    }
...
BENCHMARK(bm_sum_vector)->Range(8, 8<<10); // Benchmark on input sizes from 8 to 8192
